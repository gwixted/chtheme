...
function chtheme_setup() {
  ...
  if ( ! isset( $content_width ) ) $content_width = 640;
    register_nav_menus(
    array( 'main-menu' => __( 'Main Menu', 'chtheme' ) )
  );
}

add_action( 'wp_enqueue_scripts', 'chtheme_load_scripts' );
function chtheme_load_scripts() {  
  ...
  // wp_enqueue_style( 'wx theme styles', get_template_directory_uri() . '/css/chtheme.css', array(), '1.0', 'all');
}

add_action( 'comment_form_before', 'chtheme_enqueue_comment_reply_script' );
  function chtheme_enqueue_comment_reply_script() {
    if ( get_option( 'thread_comments' ) ) { wp_enqueue_script( 'comment-reply' ); }
}

add_filter( 'the_title', 'chtheme_title' );
function chtheme_title( $title ) {
  if ( $title == '' ) {
    return '&rarr;';
  } else {
    return $title;
...
add_filter( 'wp_title', 'chtheme_filter_wp_title' );
function chtheme_filter_wp_title( $title ) {
...
function chtheme_widgets_init() {
  register_sidebar( array (
    'name' => __( 'Sidebar Widget Area', 'chtheme' ),
...
function chtheme_custom_pings( $comment ) {
...
add_filter( 'get_comments_number', 'chtheme_comments_number' );
function chtheme_comments_number( $count ) {
  if ( !is_admin() ) {
    global $id;
